!! Z-level T/S initial-condition reader (capability A2).
module rdb_ocean_z_init
   !! Reads pre-regridded T + S from a model-grid NetCDF file
   !! (`&ocean_zinit_nml`), interpolates linearly in depth onto the
   !! seeded layer-centre depths, and writes the multilayer tracer slots
   !! as `hTr = value * h_layer`.
   !!
   !!   * File is PRE-REGRIDDED to `nx_phys x ny_phys` — no in-core
   !!     horizontal interpolation.
   !!   * Vertical interp is point-linear-in-depth, constant
   !!     extrapolation beyond the source range.
   !!   * Dry columns (`wet_mask <= 0`) get the namelist `land_fill_t/_s`
   !!     constants.
   !!
   !! Runs at seed time, HOST-side, BEFORE `ocean_state_enter_data`, so
   !! it uses plain host `do` loops (NOT `do concurrent`); the later
   !! enter_data copies the host-resident arrays up.  NetCDF-only.
...
contains
   ...
   subroutine seed_ts_from_zfile(ms, grid, cfg, ierr)
      ...
      do j = 1, ny
         do i = 1, nx
            ! Layer-centre depths (positive-down from SSH) from h_layer.
            ! Bottom-up: k=1 bed (deepest), k=nz_ml surface (shallowest).
            call build_z_ctr(ms%h_layer(ng + i, ng + j, :), nz_ml, z_ctr)
   ...
   pure subroutine build_z_ctr(h_layer, nz_ml, z_ctr)
      !! Layer-centre depths (positive-down from SSH) from a column of
      !! layer thicknesses.  Bottom-up: k=1 bed, k=nz_ml surface.
      !! `z_ctr(k) = sum_{k'=k+1..nz_ml} h(k') + 0.5*h(k)`.
      ...
      ! the total thickness of all layers shallower than k.
      above = 0.0_wp
      do k = nz_ml, 1, -1
