...
| Partial-step z-level face closure ✓ (Adcroft/Hill/Marshall 1997; Losch 2008; default off) | fields on `ocean_metrics_t` (`use_closed_faces`, `open_u`, `open_v`) + scalar latches on `ocean_vcoord_t%zfixed_closed_faces` and `ocean_vdiff_t%zlevel_faces` | builder `ocean_vcoord_closed_face_masks` + census `ocean_vcoord_count_ledges` in `vcoord/rdb_ocean_vcoord.F90`; configure in `state/rdb_ocean_setup.F90::configure_ocean_closed_faces` | 6 | `vcoord%z_top`, `dyn%bt_work%bt_H_ref`, the land-masked `dy_cu`/`dx_cv` | `open_u` (nx+1,ny,nz) / `open_v` (nx,ny+1,nz) — a STATIC per-layer 0/1 mask, 1 iff the layer is live on BOTH sides of the face under the `z_fixed` target at `η = 0`.  A 0 face is a z-LEVEL WALL for that layer: no normal velocity, no mass or tracer flux, free-slip.  **Composition rule, stated once:** `dy_eff(I,j,k) = dy_cu(I,j)·por_face_area_u(I,j,k)·open_u(I,j,k)` — land (2-D metric zeroing), porous (continuous subgrid narrowing) and z-level (per-layer closure) are INDEPENDENT factors, so porous barriers and closed faces compose rather than exclude.  Each consumer applies the two 3-D factors as SEPARATE host-gated INLINE passes rather than pre-composing them: the porous fraction is refreshed per outer step while this mask is static, and an untaken host branch that never names the array costs nothing (CLAUDE.md's +4.8 % escaping-array gotcha is about CALLS, not branches).  Consumers: the four continuity-PPM flux sites, the BT renormaliser's `wk` (a new `use_open`/`open_f` dummy pair — knob-off call sites pass an inert full-size PPM edge buffer, never the `(1,1,1)` placeholder), the two transport-Coriolis `porous_narrow_3d` sites, `mask_layer_velocities`, `tracer_hdiff_one_impl` (OPTIONAL mask dummies — it has no read-only full-size array of its own to lend), the two HARMONIC velocity-Laplacian hvisc kernels (free-slip: each neighbour difference gated by the NEIGHBOUR's flag, the tendency by the face's own), `diffuse_velocity_columns_impl` and the ALE face remap (both via a SCALAR latch + a locally derived `min(h_L,h_R) <= H_VANISHED` test — the dynamic twin of the static mask, used because neither kernel sees `ocean_metrics_t`).  `use_closed_faces = .false.` (default) ⇒ `(1,1,1)` placeholders, no branch taken, byte-identical.  BAROTROPIC CONSISTENCY is part of the slot, not a follow-up: `derive_bt_from_layers`, `face_depth_mean_u/v` (+ the `visc_rem` twins), `set_cor_ref_velocity` and `apply_bt_correction` all weight by `h_face·open`, so `ubt` is the OPEN-column depth mean and a CLOSED layer receives no barotropic increment BY CONSTRUCTION; `metrics` is a **REQUIRED** argument of the first three, of `set_cor_ref_velocity` and of `apply_bt_correction` rather than an optional one, because an optional that silently changes the physics is how the `pred_corr` Coriolis-reference defect shipped — `set_cor_ref_velocity` omitted it, took the full-column branch, and left `f·(1−φ)·v̄` forcing every barotropic substep (gate: `test_ocean_zfixed_cor_ref`); the open branch is written out in full rather than folded into the original with a runtime `if`, so the default-path kernel never NAMES `metrics` (an absent optional referenced inside a `do concurrent`, even in an untaken branch, is the kind of thing that works on the host and faults under `mem:separate`).  The OPEN layers keep whichever distribution `use_h_weighted` selected — forcing the h-weighted form concentrates the increment into the thickest open layer and was measured to blow the sloping-lid cavity up on day 2.  `dy_cu_bt`/`dx_cv_bt` carry `dy_cu·(Σ_k h_face·por·open)/(Σ_k h_face)` from `closed_faces_update_bt_widths`, refreshed from the LIVE `h` per outer step at the porous cadence and SUPERSEDING the porous write.  `bt_H_ref` is untouched — a closed face removes transport capacity, not water.  `mask_layer_velocities` also runs after the ALE remap, which is the last velocity writer of the step.  Fails loud off `z_fixed`, without a resolved `z_fixed_h_ref`, and with wet/dry, `bt_halo > 0`, GM/Redi/MLE, `nu_4 > 0`, `stress_tensor`, or the three barotropic paths still on FULL-column weights (`&ocean_bt_nml correction_bc_pgf`, `substep_drag`, `wave_drag`); or `pred_corr` with `mask_wall_velocity = .false.` (an unmasked solid-wall face's baroclinic velocity is never rotated by the `u_av`-evaluated Coriolis and grows without bound) |
