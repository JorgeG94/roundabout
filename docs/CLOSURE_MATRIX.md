...
| KPP (Large et al. 1994) | `use_kpp` (Phase 1) | overlay on PP81; `B_0` shortwave method `&ocean_thermo_nml kpp_sw_method` (all_sw/mxl_sw/lv1_sw, PR-21) | `test_ocean_kpp*`, `test_ocean_sw_bl_coupling` |
...
| FV-MOM6 in-layer T/S reconstruction (PLM/PPM Boole density integral, BOTH the per-column vertical integral and the 5-point cross-face horizontal one) | `reconstruct_for_pressure=.true.` (+ `recon_scheme=1\|2`) | `&ocean_pgf_nml`; FV_MOM6 only, default off ⇒ PCM bit-identical. **EXACT AT REST**: with a linear EOS and T/S linear in z the face acceleration is round-off (~2e-15 m/s², `C·ε·g·H/dx`) for ANY layer geometry, tilted σ layers included — the defining property of the analytic-FV PGF (Adcroft, Hallberg & Harrison 2008; Yung et al. 2026 §2.4, Fig. 5b→5c). Two pieces are load-bearing and were both missing before 2026-09-20: the boundary layers (k=1, k=nz) take a LINEAR-EXACT one-sided edge pair instead of a PCM flatten (a flatten left the full σ truncation error in exactly the layers next to the tilted boundary), and `intx_dpa`/`inty_dpa` are a 5-point Boole quadrature over sub-columns at the INTERPOLATED interface height instead of the two-column trapezoid `½(dpa_L+dpa_R)` (the trapezoid left the curvature residual `g·(−dρ/dz)·Δe²/12` at every tilted interface — the σ "second-kind" pressure-gradient error, Haney 1991). Measured on a 48×6×15 σ column over a 226→709 m linear bed: PCM 2.59e-8, interior-only reconstruction 9.43e-9, both pieces 9.6e-16 m/s². Costs 25 EOS evaluations per face per layer. **FILLER-AWARE** (2026-09-21): under a z-like coordinate the column is split into maximal runs of LIVE layers (`h > H_VANISHED`), each run reconstructed on its own with BOTH its ends closed by the linear-exact one-sided pair (so the partial cell cut by the ice base or the bed gets the same closure `k=nz` gets in the open ocean), and each vanished filler flattened to PCM edges at the adjacent live layer's edge value — its layer MEAN too, since `hTr/H_VANISHED` in a cell the ALE drain emptied is not a water property and it reached the `pa` stack, the `intx_pa` recurrence and the adjacent partial cell's own slope. All-live column ⇒ ONE run ⇒ bit-identical (asserted `==` against a verbatim copy of the unsegmented builder). Measured, `cavity_sloping_lid_rest_zfixed` + `zfixed_closed_faces`, 30 d: En 4.368e-04 → **5.042e-07**, i.e. from 187× WORSE than the PCM default (2.332e-06) to 4.6× better. What remains beneath a SLOPING ice base is the Yung et al. (2026) §3.1 surface-pressure reconstruction — `intx_pa(nz+1) = ½(pa_L+pa_R)` is a trapezoid over an edge along which `pa` is quadratic, leaving `g·γ·Δe_surf²/12` picked up with weight `(h_R−h_L)`; NOT implemented, asserted in closed form. Tests: `test_ocean_pgf_sigma_rest`, `test_ocean_pgf_reconstruct`, `test_ocean_pgf_filler_recon` |
