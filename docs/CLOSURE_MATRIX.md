...
| FV-MOM6 in-layer T/S reconstruction (PLM/PPM Boole density integral) | `reconstruct_for_pressure=.true.` (+ `recon_scheme=1\|2`) | `&ocean_pgf_nml`; FV_MOM6 only, default off ⇒ PCM bit-identical |
...
`&ocean_psurf_nml` composes freely: the liquidus reads the assembled
...
| Cover mask on ATMOSPHERIC forcing | **not implemented — REFUSED** | There is no per-cell ice-cover mask on wind stress, surface heat/salt flux, shortwave penetration or surface restoring yet, so `enable=.true.` is mutually exclusive with every one of them (fail-loud, naming the follow-up). Threading `cover_frac` as an optional argument, the way `wet_dyn` is threaded, is the next slice | `test_ocean_cavity_flux` (`validate_refusals`) |
| Diagnostics (`melt`, `thermal_driving`, `sbdry`, `ustar_shelf`, …) | **not implemented** | The slot carries every field a derived-diag catalog entry would read (`melt`, `t_b`, `s_b`, `t_far`, `s_far`, `ustar`, `q_ocean`, `status`), device-resident and mapped; the catalog entries themselves are the next slice. Absence is inert, not a hazard, so it is documented rather than refused | — |
