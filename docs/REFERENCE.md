...
| `zstar_full` | Full per-column z* | Production w/ caveat | Per-column `z_ref(0:nz)` from local bathymetry; surface layer anchored at `zstar_h_surf_target` regardless of H. Bed-side layers can vanish (`zstar_h_min`). Operators that divide by `h_layer` gate on `VANISHING_LAYER_TOL = 1.5e-4 m`. **Caveat**: geometry with frequent wet/dry cycling still leaks salt across the cycle; prefer sigma or z*-lite there. |
...
| `zstar_h_min` | 1.0e-4 | Minimum layer thickness in `zstar_full` (m); below this, the layer vanishes |
