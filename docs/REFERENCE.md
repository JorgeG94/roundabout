...
| `zfixed_closed_faces` | `.false.` | **Partial-step z-level face closure**, `z_fixed` only (Adcroft, Hill & Marshall 1997; Losch 2008 §2.1).  Treats a velocity face whose layer `k` is an inert `zstar_h_min` FILLER on EITHER side as a WALL for that layer — no normal velocity, no mass or tracer flux, free-slip — instead of leaving it open for the FV pressure gradient to integrate across a staircase step.  Builds a STATIC per-layer 0/1 face mask at configure from the `z_fixed` target at `η = 0` and composes it multiplicatively with the land metrics and the porous open-area fraction: `dy_eff = dy_cu·por·open`.  Default off ⇒ byte-identical.  **Fails loud** on any other coordinate, without a resolved `z_fixed_h_ref`, and with `&ocean_wetdry_nml enable`, `&ocean_bt_nml bt_halo > 0`, GM / Redi / Fox-Kemper MLE, `&ocean_hvisc_nml nu_4 > 0` or `stress_tensor`.  See `docs/CLOSURE_MATRIX.md` and `docs/CAPABILITIES_AND_LIMITATIONS.md`. |
| `z_fixed_profile` | `"uniform"` | Nominal layer-thickness profile of `z_fixed`: `"uniform"` (`max_depth/nz_layers` everywhere — default, byte-identical), `"list"` (`z_fixed_dz`, surface first, exactly `nz_layers` entries; a column deeper than their sum puts the excess in the bed layer) or `"tanh"` (a hyperbolic-tangent ramp in layer index from exactly `z_fixed_dz_top` at the surface, centred at `z_fixed_tanh_center` with width `z_fixed_tanh_width` — both fractions of the index span — and scaled to sum to `&ocean_topo_nml max_depth`; 6500 m / 50 levels / 2 m / 0.5 / 0.25 gives ~250 m at the bed).  Read by the `z_fixed` target builder, hence the closed-face mask, `k_top`, the partial top cell under a draft (threshold `0.1×` the straddling layer's own nominal thickness), the bed partial cell and the cavity IC seed.  **Fails loud** on any other coordinate, on a list of the wrong length, and on a tanh with `nz_layers·z_fixed_dz_top >= max_depth`.  Under a cavity the `h_min_cavity >= 2·h_nominal` rule becomes a WARNING against the thickest nominal layer. |
| `z_fixed_dz` | unset | `z_fixed_profile = "list"`: nominal thicknesses (m), surface first (MOM6 `PARAM:` / `vgrid` order). |
| `z_fixed_dz_top` | 2.0 | `z_fixed_profile = "tanh"`: surface-layer thickness (m). |
| `z_fixed_tanh_center` | 0.5 | `z_fixed_profile = "tanh"`: transition centre, fraction of the layer-index span (0 = surface). |
| `z_fixed_tanh_width` | 0.25 | `z_fixed_profile = "tanh"`: transition width, fraction of the layer-index span. |
