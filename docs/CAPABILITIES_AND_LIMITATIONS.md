...
- **`p_top` in the PGF top boundary condition** — the FV-MOM6 `pa(nz+1)` stack is still seeded at `rho_ref·g·η` and the FV-Wright / FV-lite `p_edge` stack at 0; `in_eos` moves the EOS *argument* only, never the boundary condition. Consequence under a **sloping** load: the along-layer difference `p_centre(i) − p_centre(i−1)` omits `Δp_top`. That term is depth-uniform and is already carried by the barotropic `eta_forcing` seam, so the momentum is not missing it — but a PGF that carried it directly (and a seam that then did not) is the cleaner formulation. Separate PR.
