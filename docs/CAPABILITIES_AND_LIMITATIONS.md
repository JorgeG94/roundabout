...
- **The datum half of the ice load** — `bt_H_ref = b − z_draft` (so `η ≈ 0` at rest under a draft), the `z_draft` geometry field, grounding/masking and the load-anomaly split on the `eta_forcing` seam. `p_top_in_bc` is the PGF piece only; on its own it carries no cavity concept and no static draft.
