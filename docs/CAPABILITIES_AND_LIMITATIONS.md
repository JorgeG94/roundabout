...
- **The ice-shelf cavity LOAD, wired end to end** (`&ocean_cavity_dyn_nml` + `&ocean_pgf_nml p_top_in_bc`) — `metrics%p_ice_ref = (ρ_ref·GRAVITY)·z_draft` is assembled into the top-of-column pressure
  ...
  **The payoff, measured.** `validation_examples/ocean/isomip_plus/ocean0_idealised_zfixed.nml` — ISOMIP+ Ocean0 idealised on `z_fixed` + `zfixed_closed_faces` with basal melt AND ice-shelf top drag ON, `h_min_cavity = 40 m` — **runs the full 30 days**, in 164 s on one V100 (5.5 s/simulated day), zero `nan-catch`, zero CFL truncations, `MaxCFL ≤ 0.0662`, gfortran 15.1 and the V100 agreeing to every printed digit. `En` runs `7.442E-08` (d1) → `2.628E-05` (d10) → `3.302E-04` (d30) with `max|u| ≈ 25 cm/s`; the cavity-mean melt rate runs `3.77 → 5.67 → 7.83 m/yr` with a maximum of `65.9 m/yr` and refreezing (down to `−1.5 m/yr`) appearing from day 13. That energy is the melt-driven circulation, not the coordinate's own noise: the melt-OFF twin sits at `En = 1.3E-06` at day 30. The σ twin of the same case — same namelist, `vcoord_type = "sigma"`, `h_min_cavity = 20` — **does not survive**: the barotropic loop blows up on day 3 and the melt solver fails loud on 1413 non-finite columns. That contrast is the coordinate's whole case.
