...
- **Ice-cover mask on the atmospheric forcing** (no knob of its own — it follows `&ocean_cavity_dyn_nml enable`; cavity off ⇒ byte-identical). Under `cover_frac = 1` there is no atmosphere, so every atmospheric term is multiplied by the open-water factor `1 − cover_frac` and the cavity's own `heat_cavity`/`salt_cavity` are **not**. Three places, chosen for what each one's consumers read:
  ...
  **Limitations you must read before quoting a number.**
  ...
  - **KPP and EPBL still do not see `ustar_shelf`** — unchanged by the top-drag PR, deliberately. Both derive their surface friction velocity from `stress_mag`, and under a shelf that is now **exactly zero**: the cover mask zeroes the `tau` pair on every face touching a covered cell and refreshes `stress_mag` from the masked pair in the same call. So the schemes do not mix on a phantom wind — they mix on nothing, as if the covered surface were calm. The top-drag slot PUBLISHES the quantity they should be reading instead (`stress_top`, cell-centred `|τ_top|` in N/m², device-resident) but nothing consumes it yet; blending it into `stress_mag` is a separate PR, so that one is a consumer change only.
