...
- **The ice-shelf cavity LOAD, wired end to end** (`&ocean_cavity_dyn_nml` + `&ocean_pgf_nml p_top_in_bc`) — `metrics%p_ice_ref = (ρ_ref·GRAVITY)·z_draft` is assembled into the top-of-column pressure
  ...
  at configure (final for a cavity without the psurf seam — the draft is static) and, when the psurf seam makes `sf%p_surf` live, once per outer step as an inline `do concurrent` in `ocean_dyn_step_split`. Consumers: the FV-MOM6 `pa(nz+1)` boundary condition and the in-situ EOS pressure; the forthcoming melt kernel's interface pressure is the third.
