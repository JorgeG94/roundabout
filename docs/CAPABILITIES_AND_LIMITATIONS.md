...
- **The ice-shelf cavity LOAD, wired end to end** (`&ocean_cavity_dyn_nml` + `&ocean_pgf_nml p_top_in_bc`) — `metrics%p_ice_ref = (ρ_ref·GRAVITY)·z_draft` is assembled into the top-of-column pressure
  ...
  **Fail-loud exclusions** (`configure_ocean_closed_faces`): any coordinate but `z_fixed`; an unresolved `z_fixed_h_ref` (no fillers to close); an empty `bt_H_ref`; `&ocean_wetdry_nml enable` (wet/dry moves the live/filler pattern and the mask is static); `&ocean_bt_nml bt_halo > 0` (the wide-halo BT clone carries no mask — the same argument `&ocean_porous_nml` makes); GM / Redi / Fox-Kemper MLE (all three build face fluxes from a 2-D `wet_u`/`wet_v` gate and fold them into `mass_flux_*_layer` AFTER continuity applied the per-layer mask, so their transports would leak through a closed face); the BIHARMONIC viscosity or `stress_tensor` (the free-slip closure is implemented for the harmonic velocity-Laplacian kernels only — a mirror-Neumann biharmonic pass and a `wet_q`-style corner factor each need their own derivation and test); and the three barotropic paths that still weight by the FULL column rather than the open one — `&ocean_bt_nml correction_bc_pgf` (`compute_pbce`, `compute_gtot_faces` and the bc-PGF `du_bc` block in `apply_bt_correction`: its depth-mean-zero identity holds on the full column, not on the open column `ubt` is the mean of), `substep_drag` (`compute_bt_rem`) and `wave_drag` (`compute_bt_rem_wave_drag`), both of which damp on the full-column face depth `Htot` and so under-damp a partially closed face. Each is refused until it is ported to `h_face·open`, never left to run on the wrong column. Single-rank in the same sense the porous path is.
...
- **Barotropic linear (Rayleigh) wave drag** (`&ocean_bt_nml wave_drag`,
  ...
  with `substep_drag`. Default off ⇒ bit-identical. Refused under
  `&vcoord_nml zfixed_closed_faces` (it damps on the full-column face
  depth; so is `substep_drag`). `substep_drag` itself builds its factor
...
- **Which coordinate is trustworthy on which GEOMETRY — the rest-state
  ...
  renorm_consistent_flux = .true.`) on
  gfortran 15.1 (CPU) and nvfortran 26.5 (V100), 30 simulated days per cell;
  **both toolchains agree on every verdict** (FINDING C below, the one GPU-only
  difference, is fixed).
