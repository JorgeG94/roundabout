...
- **Static ice-shelf cavity geometry + the barotropic datum** (`&ocean_cavity_dyn_nml enable`, default off ⇒ byte-identical) — a prescribed, time-constant ice draft `z_draft(i,j)` (m, positive down) laid over the bed and absorbed into the reference depth:
  ...
  **Gate:** `test_ocean_cavity_equivalence` — with a FLAT lid and no load, a cavity over a 1000 m bed evolves like a cavity-free 500 m ocean: bit-identical at t = 0, and after six `pred_corr` steps of stratified wind-driven flow the prognostics agree to 3.1e-13 m/s against a derived round-off bound of 3.1e-11, on a signal of 3.5e-3 m/s. It is NOT bitwise after stepping, and one identified term is why: the FV-MOM6 `pa` stack is an anomaly about `ρ_ref·g·z`, so shifting the column 500 m down re-rounds it (a constant offset gives exactly zero force in exact arithmetic — the theorem `p_top_in_bc` rests on — but a different rounding in floating point). Wiring the load into `pa(nz+1)` is what removes the offset. Unit coverage: `test_ocean_cavity_draft`.
...
- **The LOAD half of the ice-shelf cavity** — the datum, the `z_draft` field and grounding ship (above), but nothing consumes `metrics%p_ice_ref` yet: it is built at configure and left there. Until it is copied into `multilayer_state_t%p_top` (and thence the FV-MOM6 `pa(nz+1)` via `&ocean_pgf_nml p_top_in_bc` and the in-situ EOS via `&ocean_psurf_nml in_eos`), a cavity run is DATUM-ONLY: geometrically correct, dynamically unloaded, so a **sloping** draft is not in hydrostatic balance and will not sit still. Configure warns in those words. A FLAT lid is exact either way, which is what the equivalence gate exercises.
