...
- **Implicit stress/drag fold** (`&ocean_vdiff_nml`) — folds wind stress + bottom drag into the backward-Euler vertical-friction solve (kills thin-layer CFL blow-up).
...
- **Persistent kernel workspaces**: never local-allocate scratch in per-step kernels on `-stdpar=gpu` (stalls device on entry/exit). Use module-level allocatables, lazy-allocate via `*_workspace_ensure`, release via `*_cleanup` in the exit-data path. Use `associate` to keep in-loop names short.
