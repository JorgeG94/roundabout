...
- **The vanished-layer content rule is STRUCTURAL — don't re-derive it per kernel.** Tracers are CONTENT (`hTr = h·c`), so the tree carries one invariant: **I1 — `h <= H_VANISHED` ⇒ `hTr = 0` for every registered tracer**, restored by moving content to the nearest LIVE layer (within the column ⇒ **nothing is recorded in any budget**). Three pieces: ONE definition in `src/shared_module_utilities/rdb_vanished_layer.inc` (`rdb_vl_is_live` / `rdb_vl_conc` — *the* concentration of layer `k` — / `rdb_vl_merge_content`), `#include`d into each consuming module's `contains`; ONE enforcement point, `multilayer_state_t%enforce_vanished_content`, at the tail of `ocean_dyn_step_split` (a textual no-op on a column with no filler ⇒ sigma / z*-lite / `eulerian_z` are bit-identical); and two gates — `&vcoord_nml check_vanished_content` (fail-loud tripwire, default off, ON in every shipped namelist that vanishes layers) plus the diff-aware `vanished-layer` pre-commit hook. Substituting something OTHER than zero on a vanished layer is legal and several consumers do it deliberately (EOS → reference T/S; sponge → nearest massive layer; diagnostics → NaN; melt far-field → skip) — declare it with `! vanished-ok: <reason>`. Contract + the full consumer table: [`src/core/ocean/README.md`](src/core/ocean/README.md) §"The vanished-layer content rule". The one-sided version of this guard (read side floored, write side not) is what deleted salt/heat un-budgeted on day 16 of the ISOMIP+ z_fixed case.
