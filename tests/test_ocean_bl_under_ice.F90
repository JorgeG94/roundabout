...
contains
   ...
   subroutine test_b0_kpp(error)
      ...
      !!
      !!     B_0 = (g/rho_0)*(alpha_T*Q_heat/(rho_0*cp)
      !!                      - beta_S*Q_salt/rho_0),
      !!     B_0 > 0 STABILISING          (rdb_ocean_vmix, KPP pass 1/2)
      !!
      !! i.e. `kpp_surface_buoyancy_flux` itself — the dimensional
      !! `alpha_T`/`beta_S` of this tree cost a `1/rho_0` to become a
      !! buoyancy flux, and the expectation is built through the
      !! production helper so the units can only ever be applied once.
      !!
      ...
      checks: block
         ...
         b0_expect = kpp_surface_buoyancy_flux(ALPHA_LIN, BETA_LIN, RHO0, &
                                               q_t_kin, q_s_kin)
         ...
         call check(error, abs(b0_got - b0_expect) <= TOL_REL*abs(b0_expect), &
                    "KPP's B_0 does not match (g/rho0)*(alpha*Q_heat/(rho0*cp) "// &
